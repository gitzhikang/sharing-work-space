...
class Controller {
  constructor(targetPeerId, host, peer, broadcast, editor, doc = document, win = window) {
    ...
    this.defaultDisplayName = this.generateFallbackName(this.siteId);
    this.userName = this.loadStoredDisplayName(win) || this.defaultDisplayName;
  ...
  attachEvents(doc = document, win = window) {
    ...
    this.bindDisplayNameEvent(doc);
  }

  generateFallbackName(siteId = this.siteId) {
    return generateItemFromHash(siteId, ANIMALS);
  }

  sanitizeDisplayName(name, siteId = this.siteId) {
    const trimmed = typeof name === 'string' ? name.trim() : '';
    return trimmed || this.generateFallbackName(siteId);
  }

  loadStoredDisplayName(win = window) {
    try {
      if (!win || !win.localStorage) return null;
      const stored = win.localStorage.getItem('conclave-display-name');
      return stored ? this.sanitizeDisplayName(stored) : null;
    } catch (err) {
      return null;
    }
  }

  persistDisplayName(name, win = window) {
    try {
      if (win && win.localStorage) {
        win.localStorage.setItem('conclave-display-name', name);
      }
    } catch (err) {
      console.warn('[Controller] Unable to persist display name:', err);
    }
  }
  ...
  getPeerDisplayName(peerId) {
    ...
    if (this.broadcast && this.broadcast.peer && peerId === this.broadcast.peer.id) {
      return this.getSelfDisplayName();
    }

    const peerObj = this.network.find(obj => obj.peerId === peerId);
    if (peerObj && peerObj.name) {
      return peerObj.name;
    }
  ...
  bindDisplayNameEvent(doc = document) {
    const input = doc.querySelector('#displayNameInput');
    const saveBtn = doc.querySelector('#saveDisplayNameBtn');

    if (!input) return;

    input.value = this.getSelfDisplayName();

    const commitName = () => {
      this.updateOwnDisplayName(input.value, doc);
    };
  ...
  setDisplayNameStatus(message, doc = document, type = 'info') {
    const status = doc.querySelector('.display-name-status');
    if (!status) return;

    status.textContent = message || '';
    status.className = message ? `display-name-status ${type}` : 'display-name-status';
  ...
  updateOwnDisplayName(name, doc = document, announce = true) {
    const normalizedName = this.sanitizeDisplayName(name, this.siteId);
    const previousName = this.userName;

    this.userName = normalizedName;
    this.persistDisplayName(normalizedName, this.win);
    this.makeOwnName(doc);
    ...
    const selfNetworkEntry = this.network.find(obj => obj.siteId === this.siteId);
    if (selfNetworkEntry) {
      selfNetworkEntry.name = normalizedName;
    }

    if (this.videoRoom && this.videoRoom.refreshParticipantLabels) {
      this.videoRoom.refreshParticipantLabels();
      this.videoRoom.renderUiState();
    }

    if (announce && this.broadcast && this.broadcast.broadcastControlMessage && this.broadcast.peer && this.broadcast.peer.id) {
    ...
    if (normalizedName === previousName) {
      this.setDisplayNameStatus('Name kept as-is', doc);
    } else {
      this.setDisplayNameStatus('Name updated for this room', doc, 'success');
    }
  }
  ...
  connectToPeer(targetPeerId, doc = document) {
    ...
    this.broadcast.requestConnection(targetPeerId, this.broadcast.peer.id, this.siteId, this.getSelfDisplayName());
  ...
  addToNetwork(peerId, siteId, name = null, doc = document, announce = true) {
    const resolvedName = this.sanitizeDisplayName(name, siteId);
    const existingPeer = this.network.find(obj => obj.siteId === siteId);

    if (existingPeer) {
      existingPeer.peerId = peerId;
      existingPeer.name = siteId === this.siteId ? this.getSelfDisplayName() : resolvedName;

      if (siteId !== this.siteId) {
        this.addToListOfPeers(siteId, peerId, doc, existingPeer.name);
      } else {
        this.makeOwnName(doc);
      }
    } else {
      this.network.push({
        peerId,
        siteId,
        name: siteId === this.siteId ? this.getSelfDisplayName() : resolvedName
      });

      if (siteId !== this.siteId) {
        this.addToListOfPeers(siteId, peerId, doc, resolvedName);
      } else {
        this.makeOwnName(doc);
      }

      if (announce) {
        this.broadcast.addToNetwork(peerId, siteId, siteId === this.siteId ? this.getSelfDisplayName() : resolvedName);
      }
    }
  ...
  makeOwnName(doc = document) {
    let listItem = doc.querySelector('.self-peer-entry');
    const node = doc.createElement('span');
    const selfBadge = doc.createElement('span');
    const color = generateItemFromHash(this.siteId, CSS_COLORS);
    const name = this.getSelfDisplayName();

    node.textContent = name;
    node.style.backgroundColor = color;
    node.classList.add('peer', 'peer-name');
    selfBadge.textContent = 'You';
    selfBadge.classList.add('peer-self-note');

    if (!listItem) {
      listItem = doc.createElement('li');
      listItem.classList.add('self-peer-entry');
    ...
    listItem.appendChild(selfBadge);
  }

  addToListOfPeers(siteId, peerId, doc = document, providedName = null) {
    let listItem = doc.getElementById(peerId);
    const node = doc.createElement('span');
    ...
    const name = this.sanitizeDisplayName(providedName, siteId);
    ...
    if (!listItem) {
      ...
      const currentNameNode = listItem.querySelector('.peer-name');
      if (currentNameNode) {
        currentNameNode.textContent = name;
        currentNameNode.style.backgroundColor = color;
      }
    }
  ...
  findNewTarget() {
    ...
    if (possibleTargets.length === 0) {
      ...
      this.broadcast.requestConnection(newTarget, this.broadcast.peer.id, this.siteId, this.getSelfDisplayName());
    }
  }
  ...
  handleSync(syncObj, doc = document, win = window) {
    ...
    syncObj.network.forEach(obj => this.addToNetwork(obj.peerId, obj.siteId, obj.name, doc, false));
  ...
  handlePeerNameMessage(message, doc = document) {
    if (!message || !message.peerId || !message.siteId) return;

    const resolvedName = this.sanitizeDisplayName(message.name, message.siteId);
    const existingPeer = this.network.find(obj => obj.siteId === message.siteId);

    if (existingPeer) {
      existingPeer.peerId = message.peerId;
      existingPeer.name = resolvedName;
      if (message.siteId !== this.siteId) {
        this.addToListOfPeers(message.siteId, message.peerId, doc, resolvedName);
      }
    } else {
      this.addToNetwork(message.peerId, message.siteId, resolvedName, doc, false);
    }
