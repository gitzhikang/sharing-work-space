...
class VideoRoom {
  ...
  getPeerName(peerId, useSelfLabel = true) {
    if (!peerId) return 'Peer';

    if (this.controller && this.controller.getPeerDisplayName) {
      if (peerId === this.getSelfPeerId()) {
        return useSelfLabel ? 'You' : this.controller.getSelfDisplayName();
      }

      return this.controller.getPeerDisplayName(peerId);
    }

    if (peerId === this.getSelfPeerId()) return useSelfLabel ? 'You' : peerId;
  ...
  async enterSfuMode() {
    ...
    try {
      const token = await this.createLiveKitToken(
        ...
        this.getSelfPeerId(),
        this.getPeerName(this.getSelfPeerId(), false)
      );
  ...
  refreshParticipantLabels() {
    ...
    const tiles = stage.querySelectorAll('[data-video-peer]');
    for (let i = 0; i < tiles.length; i++) {
      const tile = tiles[i];
      const peerId = tile.dataset.videoPeer;
      const label = tile.querySelector('.video-label');

      if (label && peerId) {
        label.textContent = this.getPeerName(peerId);
      }
    }
  }
